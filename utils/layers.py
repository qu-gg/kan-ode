...
import numpy as np
from kan.spline import coef2curve, curve2coef
...
class KANLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.einsum('ij,k->ikj', x, torch.ones(self.out_dim, ).to('cuda:0')).reshape(batch, self.size).permute \
            (1, 0)
    ...
    def update_grid_from_samples(self, x):
        ...
        grid_uniform = torch.cat \
            ([grid_adaptive[:, [0]] - margin + (grid_adaptive[:, [-1]] - grid_adaptive[:, [0]] + 2 * margin) * a for a in np.linspace(0, 1, num=self.grid.shape[1])], dim=1)
    ...
    def initialize_grid_from_parent(self, parent, x):
        ...
        x_eval = torch.einsum('ij,k->ikj', x, torch.ones(self.out_dim, ).to('cuda:0')).reshape(batch, self.size).permute(1, 0)
        x_pos = parent.grid
        sp2 = KANLayer(in_dim=1, out_dim=self.size, k=1, num=x_pos.shape[1] - 1, scale_base=0.).to('cuda:0')
        ...
        percentile = torch.linspace(-1, 1, self.num + 1).to('cuda:0')
    ...
    def get_subset(self, in_id, out_id):
        ...
        spb.coef.data = self.coef.reshape(self.out_dim, self.in_dim, spb.coef.shape[1])[out_id][:, in_id].reshape(-1, spb.coef.shape
                                                                                                                      [1])
